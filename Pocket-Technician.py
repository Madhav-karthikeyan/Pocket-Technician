...
if selected_module == "Feed Tray AI":
    ...
    st.caption("Use this page for feed tray decision support and DOC-wise feed projection planning.")

    if pond["sampling_log"]:
        last_abw = float(pond["sampling_log"][-1].get("abw", 0.0))
    else:
        last_abw = 0.0
        st.info("No sampling record found yet. Feed tray logic will use ABW = 0. Add sampling data for better recommendations.")
    ...
    st.caption(
        ...
        "You can calibrate specific DOC values to reflect field changes."
    )
