...
class SamplingLog {
  const SamplingLog({
    ...
    required this.possibleExcessFeedKg,
    required this.abwG,
  ...
  final double presentNumbers;
  final double possibleExcessFeedKg;
  final double abwG;
  ...
  factory SamplingLog.fromMap(Map<String, Object?> map) => SamplingLog(
        ...
        countPerKg: (map['count_per_kg'] as num?)?.toInt() ?? 0,
        presentNumbers: (map['present_numbers'] as num?)?.toDouble() ?? 0,
        possibleExcessFeedKg: (map['possible_excess_feed_kg'] as num?)?.toDouble() ?? 0,
